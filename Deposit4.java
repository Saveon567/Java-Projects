...
public class Deposit4 {
	...
	public static void main(String[] args) {
		...
		//the amount you put into your account each month. 
		//Read in from the keyboard
		double monthlyDeposit;
		
		//the yearly interest rate. Read in from the keyboard.
		double yearlyIntRate;
		
		//computed as yearlyIntRate/100/12
		...
		//initialized to 0. Stores the current value of the account at the end of the month using the formula:
		//monthlyValue = (monthlyDeposit + monthlyValue) * (1 + monthlyIntRate); 
		...
		//Deposit Amount Input & Output
		System.out.println("Please Enter The Deposit Amount: ");
	    monthlyDeposit = input.nextDouble();
	     
	    //Interest Input & Output
	    System.out.println("Enter interest rate: ");
	    yearlyIntRate = input.nextDouble();
	    monthlyIntRate = (yearlyIntRate / 100)/12;
	    
	    double January =	monthlyValue = (monthlyDeposit + monthlyValue) * (1 + monthlyIntRate); 
	    January = (int)(January*100)/100.0;
	    double February =	monthlyValue = (monthlyDeposit + monthlyValue) * (1 + monthlyIntRate); 
	    February = (int)(February*100)/100.0;
	    double March =	monthlyValue = (monthlyDeposit + monthlyValue) * (1 + monthlyIntRate); 
	    March = (int)(March*100)/100.0;
	    double April =	monthlyValue = (monthlyDeposit + monthlyValue) * (1 + monthlyIntRate); 
	    April = (int)(April*100)/100.0;
	    double May =	monthlyValue = (monthlyDeposit + monthlyValue) * (1 + monthlyIntRate); 
	    May = (int)(May*100)/100.0;
	    double June =	monthlyValue = (monthlyDeposit + monthlyValue) * (1 + monthlyIntRate); 
	    June = (int)(June*100)/100.0;
	    
	    System.out.println(" January Value Is  "  + January);
	    System.out.println(" February Value Is  " + February);
	    System.out.println(" March Value Is  "    + March);
	    System.out.println(" April Value Is  "    + April);
	    System.out.println(" May Value Is  "  + May);
	    System.out.println(" June Value Is  "  + June);
	    }
	}
